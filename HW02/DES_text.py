...
def decrypt(inputFile, roundKeyFile, outputFile):
	with open(inputFile) as IP:
		bv = BitVector(hexstring = IP.read())
	...
	reversed_roundKeys = round_keys[::-1]

	i  = 64
	stre = ""
	for i in range(64, 9025, 64):
		bitvec = bv[i-64:i]
		[LE, RE] = bitvec.divide_into_two() #divide into 2 halves

		for round_key in reversed_roundKeys: #run through all round keys
		...
		stre += (block_cipher.get_bitvector_in_ascii())
		i+=64
...
def encrypt(inputFile, roundKeyFile, outputFile): #INPUT FILE IS ALREADY OPENED, OUTPUT FILE MUST BE OPENED AND WRITTEN TO IN THIS FUNCTION
	...
	while (bv.more_to_read):
		...
		if bitvec.length() > 0: #need to add extra zeros
			...
			block_cipher = RE + LE #combine the halves
			if sys.argv[1] == "-e":
				#block_cipher.write_to_file(file_out)
				hex_block = block_cipher.get_bitvector_in_hex()
